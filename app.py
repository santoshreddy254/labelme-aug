...
from aug_gui import aig_window_2
...
class MainWindow(QtWidgets.QMainWindow):
    ...
    def __init__(
        ...
        augment_ = action(
            ...
            None,
            "augment",
            self.tr("Augment the annotated images"),
            enabled=False,
        )
    ...
    def update_labels_file(self,labels_list_to_save):
        file1 = open(self.save_folder_2+"/labels.txt", "w")
        file1.write("__ignore__ \n_background_\n")
        for i in labels_list_to_save:
            file1.write(i+"\n")
        file1.close()
    ...
    def openNextImg(self, _value=False, load=True):
        ...
        self._config["keep_prev"] = keep_prev
        if self.uniqLabelList.count()>0:
            labels_list_to_save = []
            for i in range(self.uniqLabelList.count()):
                label_i = self.uniqLabelList.item(i).data(Qt.UserRole)
                labels_list_to_save.append(label_i)
            self.update_labels_file(labels_list_to_save)
    ...
    def changeOutputDirDialog(self, _value=False):
        ...
        if current_filename in self.imageList:
            ...
            self.fileListWidget.repaint()
        self.save_folder_2 = self.output_dir
    ...
    def augment(self):
        ...
        convert_to_voc(self.source_folder, self.save_folder_2,
                       self.save_folder_2+"/labels.txt")
        generate_semantic_labels(self.save_folder_2+"/voc_data", self.save_folder_2)
        generator_options = GeneratorOptions()
        generator_options.set_image_path(self.save_folder_2+"/voc_data/JPEGImages")
        generator_options.set_label_path(self.save_folder_2+"/semantic_labels")
        generator_options.set_labels_file_path(self.save_folder_2+"/labels.txt")
    ...
    def openDirDialog(self, _value=False, dirpath=None):
        ...
        self.source_folder = targetDirPath
        self.save_folder_2 = targetDirPath
    ...
    def importDirImages(self, dirpath, pattern=None, load=True):
        ...
        if not self.mayContinue() or not dirpath:
            return
        self.annotations_list_check_augment = []
